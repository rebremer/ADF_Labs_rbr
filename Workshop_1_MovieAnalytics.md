...
1. **Create a new DelimitedText dataset to use as a source**
    ...
    1. Select the Azure Blob Storage title and click continue
...
1. **Create a new DelimitedText dataset to use as a sink**
    ...
    ![Authoring](./assets/MovieAnalytics/authoring6.png)
    1. In the Set Properties sidenav, give your dataset an understandable name and create a new `ADLS` linked service. Select the storage account you created in the prerequisites and use account key as authentication.
   ![Authoring](./new_images_rbr/4_adls_linked_service.png)    
...
1. **Rank movies via a Window Transformation** Say you are interested in how a movie ranks within its year for its specific genre. You can add a [Window transformation](https://docs.microsoft.com/azure/data-factory/data-flow-window) to define window-based aggregations by clicking on the + icon next to your Derived Column transformation and clicking Window under Schema modifier.
    ...
    Next, go to the **Sort** tab and specify a sort order for each window group. Because we are interested in ranking the highest rated movies according to `Rotten Tomato` first, the sort column is `Rotten Tomato` and the order is descending.
...
1. **Write to an Azure SQL Database** Once you have finished your transformation logic, you are ready to write to your sink. In this lab, you are going to create a new Azure SQL Sink. To add a sink, click on the + icon next to your Aggregate transformation and select Sink.

    ![derive](./assets/MovieAnalytics/synapse-sink1.PNG "derive")

    Name your sink `SQLSink` and select a new dataset.

    ![derive](./assets/MovieAnalytics/synapse-sink2.PNG "derive")

    Select the Azure SQL Database tile.

    ![derive](./new_images_rbr/5_Azure_SQL_Database.png "derive")

    Name your dataset `SQLSink` and select a new linked service.

    ![derive](./new_images_rbr/6_Azure_SQL_Database_dataset.png "derive")

    Name your linked service `AzureSQL` and enter in your credentials. The below example uses SQL authentication. Click OK when finished.
...
![Data Flow Canvas](./new_images_rbr/10_dataflows_pipeline.png "Data Flow Canvas")
